from src.CDBH import cdbh_functions_tcc as cf
...
class tcc_cdbh():
    ...
    def fit(self):
        ...
        df_train = pd.merge(pipe_prep.transform(X_train), pd.DataFrame(y_train),left_index=True, right_index=True, how = 'inner')
        
        cdbhzer = cf.cdbh_functions(dataframe = df_train, df_train=df_train)
        ...
        LGBM_bayes_search.fit(X_train_cdbh, y_train_cdbh)        
        ...
        best_LGBM.fit(pipe_prep.transform(X_train_cdbh), y_train_cdbh, early_stopping_rounds = 10, verbose = 20, eval_metric = metric,
